...
/**
  ...
  */
object P50Fields_Complex_20Gb_Transform {

  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      .builder()
      .appName("P50Fields_Complex_20Gb_Transform").master("yarn")
    ...
    val df = spark.sql("From TransForm Select *,"
      ...
      + "substring(country,1) AS changed_country,"
      + "substring(state,1) AS changed_state,"
      + "substring(stateProvince,1) AS changed_stateProvince,"
      + "substring(maritalStatus,1) AS changed_maritalStatus,"
      + "upper(lower(trim(substring(applyingFor,1)))) AS changed_citizenshipOfIndiaBy,"
      + "upper(lower(trim(substring(employmentType,1)))) AS changed_employmentType,"
      + "upper(lower(trim(substring(organizationName,1)))) AS changed_organizationName,"
      + "voteId + 21 AS changed_voteId,"
      + "PAN +21 AS changed_PAN,"
      + "mobileNumber *21 AS changed_mobileNumber,"
      + "telephoneNumber *21 AS changed_telephoneNumber,"
      + "fatherGuardianFilePassportNumber * 21.0  AS changed_fatherGuardianFilePassportNumber,"
      + "fileNumber% 3 AS changed_fileNumber,"
      + "date_add(dDissueDate, 14562) AS changed_dDissueDate,"
      + "date_add(dDExpiryDate, 35458) AS changed_dDExpiryDate"
    )
    ...
    df.write
      ...
      .csv("testData/benchmarking/output/50Fields_Complex_20Gb_transform_out")
  }
