...
/**
  ...
  */
object P50Fields_Simple_20Gb_Sort {
  def main(args: Array[String]): Unit = {
   ...
   val sortedDf = spark.sql("SELECT  * FROM DataFrame order by PAN asc")

    sortedDf.write
      .option("timestampFormat","yyyy/MM/dd hh:mm:ss")
      .option("dateFormat","yyyy/MM/dd")
      .mode(SaveMode.Overwrite).csv("testData/benchmarking/output/50Fields_Simple_20Gb_Sort")
  }

  def getSparkSession: SparkSession = {
    SparkSession
      .builder()
      .appName("P50Fields_Simple_20Gb_Sort").master("yarn")
