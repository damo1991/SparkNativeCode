...
/**
  ...
  */
object P50Fields_Simple_20Gb_Join {
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession
      .builder()
      .appName("P50Fields_Simple_20Gb_Join").master("yarn")
    ...
    joinquery.select(outwardSchema: _*).write
      ...
      .csv("testData/benchmarking/output/50Fields_Simple_20Gb_Join")

    val unsed1 = sparkSession
      .sql("SELECT * FROM inputtable0 in0 where in0.PAN not in (select new_pan from joinresult)")

    val unsed2 = sparkSession
      .sql("SELECT * FROM inputtable1 in1 where in1.PAN not in (select new_pan from joinresult)")
    ...
    unsed2.select(unusedSchema2: _*).write
      ...
      .option("timestampFormat","yyyy/MM/dd hh:mm:ss")
      .mode(SaveMode.Overwrite)
      .csv("testData/benchmarking/output/50Fields_Simple_20Gb_Join_unused0")
    ...
    unsed1.select(unusedSchema1: _*).write
      ...
      .option("timestampFormat","yyyy/MM/dd hh:mm:ss")
      .mode(SaveMode.Overwrite)
      .csv("testData/benchmarking/output/50Fields_Simple_20Gb_Join_unused1")
