...
/**
  ...
  */
object P50Fields_Complex_20Gb_Lookup {
  def main(args: Array[String]): Unit = {

    val sparkSession = SparkSession
      .builder()
      .appName("P50Fields_Complex_20Gb_Lookup").master("yarn")
    ...
    val otherFields: Array[Column] = readFile2.columns.filter(col => !col.equalsIgnoreCase("PAN") && !col.equalsIgnoreCase("organizationName") && !col.equalsIgnoreCase("surnameapplicant") && !col.equalsIgnoreCase("givennameapplicant")).map(str => last(str).as(str))
    val broadCastDF = broadcast(readFile2.groupBy("PAN","givennameapplicant","surnameapplicant","organizationName").agg(lit(null),otherFields:_*))
    ...
    joinquery.select(outwardSchema: _*).write
      ...
      .csv("testData/benchmarking/output/50Fields_Complex_20Gb_Lookup")
  }
}
