...
/**
  ...
  */
object P50Fields_Medium_20Gb_Filter {
  def main(args: Array[String]): Unit = {
    ...
    val used = spark.sql("SELECT  * FROM DataFrame WHERE applyingFor = 'Fresh' AND apptype='Tatkal' AND bookletType='60Pages' AND educationalQualification='10thPassAndAbove' AND nonECR = 'True' AND (ecDateofIssue > '2008/01/01')")
    val unused = spark.sql("SELECT  * FROM DataFrame WHERE applyingFor != 'Fresh' OR apptype !='Tatkal' OR bookletType!='60Pages' OR educationalQualification!='10thPassAndAbove' OR nonECR != 'True' OR !(ecDateofIssue > '2008/01/01')")

    used.write
      .option("timestampFormat","yyyy/MM/dd hh:mm:ss")
      .option("dateFormat","yyyy/MM/dd")
      .mode(SaveMode.Overwrite).csv("testData/benchmarking/output/50Fields_Medium_20Gb_Filter_out")
    unused.write
      .option("timestampFormat","yyyy/MM/dd hh:mm:ss")
      .option("dateFormat","yyyy/MM/dd")
      .mode(SaveMode.Overwrite).csv("testData/benchmarking/output/50Fields_Medium_20Gb_Filter_unused")
  }

  def getSparkSession: SparkSession = {
    SparkSession
      .builder()
      .appName("P50Fields_Medium_20Gb_Filter").master("yarn")
